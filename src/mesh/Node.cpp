...
void Node::PrintDeviceType(const joinMeBufferPacket& packet) const {
    ...
    //trace("ClusterId: %d" EOL, packet.payload.clusterId);
}
...
TerminalCommandHandlerReturnType Node::TerminalCommandHandler(const char* commandArgs[], u8 commandArgsSize)
{
    ...
    if(commandArgsSize >= 3 && TERMARGS(2 , "node"))
    {
        if(TERMARGS(0 ,"action"))
        {
            ...
            //new command: set priority
			if(commandArgsSize > 3 && TERMARGS(3, "set_prio"))
			{
				//  0     1    2      3      4
				//action this node set_prio 1/0
				highPriority = Utility::StringToU8 (commandArgs[4]);
				// if(highPriority == 1)
				// 	nodeType = DeviceType::PRIO;
				// else
				// 	nodeType = DeviceType::STATIC;
				printf("prio of node %d set to %d\n",destinationNode, highPriority);
				return TerminalCommandHandlerReturnType::SUCCESS;
			}

			//new command: set sink
			if(commandArgsSize > 3 && TERMARGS(3, "set_type"))
			{
				//  0     1    2      3         4
				//action this node set_type deviceType
				u8 temp = Utility::StringToU8 (commandArgs[4]);
				if(temp == 3)
					nodeType = DeviceType::SINK;
				else if(temp == 1)
					nodeType = DeviceType::STATIC;
				printf("type of node %d set to %d\n",destinationNode, temp);
				return TerminalCommandHandlerReturnType::SUCCESS;
			}
